...
def inside_ccw(p, a, b, c):
    return (
        cross2d(a - p, b - p) >= 0
        and cross2d(b - p, c - p) >= 0
        and cross2d(c - p, a - p) >= 0
    )
...
class Voxelizer:
    def __init__(self, res, dx, super_sample=2, precision=ti.f64, padding=3):
        ...
        # Super sample by 2x
        self.res = (res[0] * super_sample, res[1] * super_sample, res[2] * super_sample)
    ...
    @ti.kernel
    def voxelize_triangles(self, num_triangles: ti.i32, triangles: ti.types.ndarray()):
        for i in range(num_triangles):
            ...
            jitter = (
                ti.Vector(
                    [-0.057616723909439505, -0.25608986292614977, 0.06716309129743714]
                )
                * jitter_scale
            )
            ...
            for p in range(p_min, p_max):
                for q in range(q_min, q_max):
                    ...
                    if inside_ccw(pos2d, a_proj, b_proj, c_proj) or inside_ccw(
                        ...
                        height = int(
                            -normal.dot(base_voxel - a) / normal[2] * self.inv_dx + 0.5
                        )

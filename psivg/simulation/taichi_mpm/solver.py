...
class MPMSolver:
    ...
    def __init__(
        ...
        self.default_dt = 2e-2 / 64 * dt_scale
        ...
        for g in range(self.num_grids):
            ...
            block.dynamic(
                ti.axes(self.dim),
                1024 * 1024,
                chunk_size=self.leaf_block_size**self.dim * 8,
            ).place(pid, offset=block_offset + (0,))

        self.padding = padding

        # Young's modulus and Poisson's ratio
        self.use_diff_E = use_diff_E
        if self.use_diff_E:
            self.E = ti.field(dtype=ti.f32)
            self.nu = ti.field(dtype=ti.f32)
        self.E_0, self.nu_0 = 1e6 * size * E_scale, 0.2
        # Lame parameters
        self.mu_0, self.lambda_0 = self.E_0 / (
            2 * (1 + self.nu_0)
        ), self.E_0 * self.nu_0 / ((1 + self.nu_0) * (1 - 2 * self.nu_0))
        ...
        if self.quant:
            ...
            if self.use_diff_E:
                self.particle.place(self.E, self.nu)
            ...
            self.particle.place(self.color, self.rho)
            if self.use_emitter_id:
            ...
            if self.use_emitter_id:
                ...
                self.particle.place(
                    self.x, self.v, self.F, self.material, self.color, self.rho
                )
            if self.support_plasticity:
                self.particle.place(self.Jp)
            if not self.use_g2p2g:
    ...
    def sand_projection(self, sigma, p):
        ...
        if tr >= 0.0:
            ...
            delta_gamma = (
                epsilon_hat_norm
                + (self.dim * self.lambda_0 + 2 * self.mu_0)
                / (2 * self.mu_0)
                * tr
                * self.alpha
            )
            for i in ti.static(range(self.dim)):
                sigma_out[i, i] = ti.exp(
    ...
    @ti.kernel
    def build_pid(
        self, pid: ti.template(), grid_m: ti.template(), offset: ti.template()
    ):
        """
        ...
        for p in self.x:
            base = int(ti.floor(self.x[p] * self.inv_dx - 0.5)) - ti.Vector(self.offset)
    ...
    def g2p2g(
        ...
        grid_v_out: ti.template(),
        grid_m_out: ti.template(),
    ):
        ...
        for I in ti.grouped(pid):
            ...
            if self.material[p] != self.material_stationary:
                self.v[p] = new_v
                self.v[p] *= ti.exp(-self.drag_damping * dt)
            ...
            # Lame parameters
            mu, la = 0.0, 0.0
            if ti.static(self.use_diff_E):
                mu = self.E[p] / (2 * (1 + self.nu[p])) * h
                la = (
                    self.E[p]
                    * self.nu[p]
                    / ((1 + self.nu[p]) * (1 - 2 * self.nu[p]))
                    * h
                )
            else:
            ...
            if self.material[p] != self.material_sand:
                stress = 2 * mu * (self.F[p] - U @ V.transpose()) @ self.F[
                    p
                ].transpose() + ti.Matrix.identity(ti.f32, self.dim) * la * J * (J - 1)
            else:
                if ti.static(self.support_plasticity):
                    ...
                    for i in ti.static(range(self.dim)):
                        ...
                        center[i, i] = (
                            2.0 * self.mu_0 * ti.log(sig[i, i]) * (1 / sig[i, i])
                        )
                    for i in ti.static(range(self.dim)):
            ...
            affine = stress + self.rho[p] * self.p_vol * C
            ...
            for offset in ti.static(ti.grouped(self.stencil_range())):
                ...
                grid_v_out[base + offset] += weight * (
                    self.rho[p] * self.p_vol * self.v[p] + affine @ dpos
                )
                grid_m_out[base + offset] += weight * self.rho[p] * self.p_vol
    ...
    def p2g(self, dt: ti.f32):
        ...
        for I in ti.grouped(self.pid):
            ...
            if self.material[p] != self.material_sand:
                stress = 2 * mu * (
                    F - U @ V.transpose()
                ) @ F.transpose() + ti.Matrix.identity(ti.f32, self.dim) * la * J * (
                    J - 1
                )
            else:
                if ti.static(self.support_plasticity):
            ...
            mass = self.rho[p] * self.p_vol
            if self.material[p] == self.material_water:
                mass *= self.water_density
            affine = stress + mass * self.C[p]
            # if self.material[p] == self.material_stationary and p % 1000 == 0:
            #     print('stationary', affine)
            # if self.material[p] == self.material_elastic and p % 1000 == 0:
            #     print('elastic', affine)
            # Loop over 3x3 grid node neighborhood
            for offset in ti.static(ti.grouped(self.stencil_range())):
                dpos = (offset.cast(float) - fx) * self.dx
                weight = 1.0
                ...
                self.grid_v[base + offset] += weight * (
                    mass * self.v[p] + affine @ dpos
                )
                self.grid_m[base + offset] += weight * mass

    @ti.kernel
    def p2g_1(self, dt: ti.f32):
        ti.no_activate(self.particle)
        ti.loop_config(block_dim=256)
        if ti.static(self.use_bls):
            for d in ti.static(range(self.dim)):
    ...
    def grid_normalization_and_gravity(
        ...
        for I in ti.grouped(grid_m):
            ...
            if ti.static(
                self.g2p2g_allowed_cfl > 0 and self.use_g2p2g and self.v_clamp_g2p2g
            ):
        ...
        for I in ti.grouped(grid_v):
            for d in ti.static(range(self.dim)):
                if ti.static(unbounded):
                    if I[d] < -self.grid_size // 2 + self.padding and grid_v[I][d] < 0:
    ...
    def add_surface_collider(
        self, point, normal, surface=surface_sticky, friction=0.0, rebound=0.0
    ):
        point = list(point)
        # Normalize normal
        ...
        if surface == self.surface_sticky and (friction != 0 or rebound != 0):
            raise ValueError("friction and rebound must be 0 on sticky surfaces.")
            ...
            for I in ti.grouped(grid_v):
                ...
                if offset.dot(n) < 0:
                    if ti.static(surface == self.surface_sticky):
                        ...
                        if ti.static(surface == self.surface_slip):
                            # Project out all normal component
                            v = v - n * normal_component * (1 + rebound)
                        else:
                            # Project out only inward normal component
                            v = v - n * min(normal_component, 0) * (1 + rebound)

                        if normal_component < 0 and v.norm() > 1e-30:
                            # Apply friction here
                            v = v.normalized() * max(
                                0,
                                v.norm() + normal_component * (1 + rebound) * friction,
                            )
                        grid_v[I] = v
    ...
    def add_bounding_box(self, unbounded):
        self.grid_postprocess.append(
            lambda t, dt, grid_v: self.grid_bounding_box(t, dt, unbounded, grid_v)
        )
    ...
    @ti.kernel
    def compute_max_displace(self) -> ti.f32:
        max_displace = 0.0
        for p in self.x:
            x = self.x[p]
            x_max = 0.0
            ...
            ti.atomic_max(max_displace, x_max)
        return max_displace
    ...
    def step(self, frame_dt, print_stat=False, smry_writer=None, frame=None):
        begin_t = time.time()
        ...
        while frame_time_left > 0:
            ...
            if self.use_g2p2g:
                ...
                self.build_pid(
                    self.pid[self.input_grid], self.grid_m[self.input_grid], 0.5
                )
                self.g2p2g(
                    dt,
                    self.pid[self.input_grid],
                    self.grid_v[self.input_grid],
                    self.grid_v[output_grid],
                    self.grid_m[output_grid],
                )
                self.grid_normalization_and_gravity(
                    dt, self.grid_v[output_grid], self.grid_m[output_grid]
                )
                ...
                # if frame==13 and dot_count in [22, 23, 24, 25]:
                #     self.p2g_1(dt)
                #     print("done")
                # else:
                self.p2g(dt)
            ...
            if smry_writer is not None:
                smry_writer.add_scalar(
                    "substep_max_CFL",
                    cur_frame_velocity * dt / self.dx,
                    self.total_substeps,
                )
            self.all_time_max_velocity = max(
                self.all_time_max_velocity, cur_frame_velocity
            )
    ...
    def seed(
        ...
        for i in range(self.n_particles[None], self.n_particles[None] + new_particles):
            ...
            self.seed_particle(
                i, x, new_material, color, self.source_velocity[None], None
            )
    ...
    def add_cube(
        ...
        E = E * self.size * self.E_scale
        self.seed(num_new_particles, material, color, E, nu, rho)
    ...
    def add_ngon(
        ...
        num_particles = (
            0.5 * (radius * self.inv_dx) ** 2 * math.sin(2 * math.pi / sides) * sides
        )
    ...
    def random_point_in_unit_polygon(self, sides, angle):
        ...
        while True:
            ...
            theta = (
                point_angle - angle
            ) % central_angle  # polygon angle is from +X axis
            phi = central_angle / 2
    ...
    def add_texture_2d(
        ...
        for i, j in ti.ndrange(texture.shape[0], texture.shape[1]):
            if texture[i, j] > 0.1:
                ...
                self.seed_particle(
                    pid, x, new_material, color, self.source_velocity[None], None
                )
    ...
    def seed_ellipsoid(
        ...
        for i in range(self.n_particles[None], self.n_particles[None] + new_particles):
            ...
            x = (
                self.source_bound[0]
                + self.random_point_in_unit_sphere() * self.source_bound[1]
            )
            self.seed_particle(
                i, x, new_material, color, self.source_velocity[None], None
            )
    ...
    def add_ellipsoid(
        ...
        E = E * self.size * self.E_scale
        self.seed_ellipsoid(num_particles, material, color, E, nu, rho)
        self.n_particles[None] += num_particles
    ...
    def seed_from_voxels(
        ...
        for i, j, k in self.voxelizer.voxels:
            ...
            for d in ti.static(range(3)):
                inside = (
                    inside
                    and -self.grid_size // 2 + self.padding <= i
                    and i < self.grid_size // 2 - self.padding
                )
            if inside and self.voxelizer.voxels[i, j, k] > 0:
                ...
                for l in range(sample_density + 1):
                    if ti.random() + l < s:
                        x = (
                            ti.Vector(
                                [ti.random() + i, ti.random() + j, ti.random() + k]
                            )
    ...
    def add_mesh(
        ...
        E = E * self.size * self.E_scale
        self.seed_from_voxels(material, color, sample_density, emmiter_id, E, nu, rho)
        ti.sync()

    @ti.kernel
    def seed_from_external_array(
        self,
        ...
        for i in range(num_particles):
            ...
            p = self.n_particles[None] + i
            if ti.static(self.use_diff_E):
                self.E[p] = E
                self.nu[p] = nu
            self.rho[p] = rho
            self.seed_particle(
                p, x, new_material, color, self.source_velocity[None], None
            )
        self.n_particles[None] += num_particles

    @ti.kernel
    def seed_from_external_array_colors(
        self,
        num_particles: ti.i32,
        pos: ti.types.ndarray(),
        new_material: ti.i32,
        ...
        for i in range(num_particles):
            ...
            self.rho[p] = rho
            self.seed_particle(
                p, x, new_material, colors[i], self.source_velocity[None], None
            )

        self.n_particles[None] += num_particles

    def add_particles_with_velocities(
        self,
        particles,
        velocities,
        colors,
        ...
        """
        Add particles with individual velocities for each particle.

        Args:
            particles: numpy array of shape (N, dim) with particle positions
            velocities: numpy array of shape (N, dim) with individual velocities
            colors: numpy array of shape (N,) with particle colors
            material: material type for all particles
            E: Young's modulus (stiffness)
            nu: Poisson's ratio (ratio of lateral to axial strain)
            rho: density (mass per unit volume)
        """
        assert len(particles) == len(
            velocities
        ), f"Number of particles ({len(particles)}) and velocities ({len(velocities)}) must match"
        assert len(particles) == len(
            colors
        ), "Number of particles and colors must match"

        # Ensure correct data types to avoid precision warnings
        particles = np.asarray(particles, dtype=np.float32)
        velocities = np.asarray(velocities, dtype=np.float32)
        colors = np.asarray(colors, dtype=np.int32)

        # Use the new method that supports per-particle velocities with material properties
        self.seed_from_external_array_with_velocities(
    ...
    @ti.kernel
    def seed_from_external_array_with_velocities(
        self,
        num_particles: ti.i32,
        pos: ti.types.ndarray(),
        ...
        for i in range(num_particles):
            ...
            # unpacking the position and velocity
            if ti.static(self.dim == 3):
                # For 3D simulation
                x = ti.Vector([pos[i, 0], pos[i, 1], pos[i, 2]])
            ...
            # Each new particle gets a unique index
            # self.n_particles[None] serves as a global scalar counter
            p = self.n_particles[None] + i
            ...
            self.seed_particle(
                i=p,  # particle index
                x=x,  # position
                material=new_material,  # material type
                color=colors[i],  # color
                velocity=v,  # velocity
                emmiter_id=None,  # emitter id
            )
        self.n_particles[None] += num_particles
    ...
    def overwrite_velocities(
        ...
        """
        Overwrite velocities for particles in the range [start_index, end_index).

        Args:
            start_index: Starting particle index (inclusive)
            end_index: Ending particle index (exclusive)
            velocities: Array of velocities with shape (num_particles, dim)
                       where num_particles = end_index - start_index
        """
        # Input validation
        if start_index < 0 or end_index > self.n_particles[None]:
            raise ValueError(
                f"Index range [{start_index}, {end_index}) out of bounds. "
                f"Valid range: [0, {self.n_particles[None]})"
            )

        if start_index >= end_index:
            raise ValueError(
                f"Invalid index range: start_index ({start_index}) >= end_index ({end_index})"
            )

        num_particles = end_index - start_index

        # Validate velocities array shape
        if velocities.shape[0] != num_particles:
            raise ValueError(
                f"Expected velocities array with {num_particles} particles, "
                f"got {velocities.shape[0]}"
            )

        if velocities.shape[1] != self.dim:
            raise ValueError(
                f"Expected velocities array with {self.dim} dimensions, "
                f"got {velocities.shape[1]}"
            )

        # Use Taichi kernel for better performance
        self._overwrite_velocities_kernel(
            start_index, end_index, np.ascontiguousarray(velocities, dtype=np.float32)
        )

    @ti.kernel
    def _overwrite_velocities_kernel(
        self,
        start_index: ti.i32,
        end_index: ti.i32,
        velocities: ti.types.ndarray(),
    ):
        """Taichi kernel to overwrite particle velocities."""
        for i in range(start_index, end_index):
            v_i = i - start_index  # Index into velocities array
            v = ti.Vector.zero(ti.f32, self.dim)
            for d in ti.static(range(self.dim)):
                v[d] = velocities[v_i, d]
            self.v[i] = v
    ...
    def add_particles(
        ...
        self.set_source_velocity(velocity=velocity)
        self.seed_from_external_array(
            len(particles), particles, material, color, E, nu, rho
        )

    def add_particles_color(
        self, particles, colors, material, velocity=None, E=1e6, nu=0.2, rho=1000
    ):
        self.set_source_velocity(velocity=velocity)
        self.seed_from_external_array_colors(
            len(particles), particles, material, colors, E, nu, rho
        )
    ...
    def recover_from_external_array(
        ...
        for i in range(num_particles):
            ...
            if ti.static(self.dim == 3):
                ...
                v = ti.Vector([vel[i, 0], vel[i, 1]])
            self.seed_particle(
                self.n_particles[None] + i, x, material[i], color[i], v, None
            )
        self.n_particles[None] += num_particles

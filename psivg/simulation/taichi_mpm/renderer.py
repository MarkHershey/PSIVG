...
class Renderer:
    def __init__(
        ...
        self.particle_bucket.dense(ti.ijk, self.block_size).dynamic(
            ti.l, self.max_num_particles_per_cell, chunk_size=32
        ).place(self.pid, offset=self.particle_grid_offset + [0])
        ...
        ti.root.pointer(ti.ijk, self.particle_grid_res // self.block_size).dense(
            ti.ijk, self.block_size
        ).place(self.voxel_has_particle, offset=voxel_grid_offset)
        voxel_block = ti.root.pointer(ti.ijk, self.voxel_grid_res // self.block_size)

        voxel_block.dense(ti.ijk, self.block_size).place(
            self.voxel_grid_density, offset=voxel_grid_offset
        )
    ...
    def inside_grid_loose(self, ipos):
        return (
            ipos.min() >= -self.voxel_grid_res // 2 - 1
            and ipos.max() <= self.voxel_grid_res // 2
        )
    ...
    def sdf(self, o):
        ...
        if ti.static(self.supporter == 0):
            ...
            dist = (
                min(max(d[0], d[1]), 0.0)
                + ti.Vector([max(d[0], 0.0), max(d[1], 0)]).norm()
                - rb
            )
    ...
    def render(self):
        ...
        for u, v in self.color_buffer:
            ...
            fu = 2 * fov * (u + ti.random(ti.f32)) / res[1] - fov * aspect_ratio - 1e-5
                    ...
                    for k in range(offset_begin_grid[2], offset_end_grid[2]):
                        ...
                        if self.inside_particle_grid(box_ipos):
                            ...
                            if sphere_aabb_intersect_motion(
                                ...
                                ti.append(
                                    self.pid.parent(),
                                    box_ipos - ti.Vector(self.particle_grid_offset),
                                    p,
                                )
    ...
    def initialize_particles_from_taichi_elements(self, particle_fn):
        ...
        for i in range(3):
            ...
            self.bbox[0][i] = (
                math.floor(np_x[:, i].min() * self.inv_dx) - 3.0
            ) * self.dx
            self.bbox[1][i] = (
                math.floor(np_x[:, i].max() * self.inv_dx) + 3.0
            ) * self.dx
        ...
        for i in range(num_slices):
            ...
            self.initialize_particle(
                np_x[begin:end], np_v[begin:end], np_color[begin:end], begin, end
            )

...
def rotate_mesh(vertices, angle_x=0, angle_y=0, angle_z=0):
    ...
    Ry = np.array(
        [
            [np.cos(radians_y), 0, np.sin(radians_y)],
            [0, 1, 0],
    ...
    Rz = np.array(
        [
            [np.cos(radians_z), -np.sin(radians_z), 0],
...
def load_mesh_ply(fn, scale, offset, reduction_rate=1, angle_x=0, angle_y=0, angle_z=0):
    ...
    for face in elements["vertex_indices"]:
        if len(face) == 3:
            ...
            raise ValueError(
                "The mesh contains polygons with more than 4 vertices, which is not supported."
            )
    ...
    print(
        "loaded and simplified, face count:", len(faces), "vertex count:", len(vertices)
    )

    return triangles


def sample_point_mesh(
    fn, scale, offset, sample_rate=1, angle_x=0, angle_y=0, angle_z=0
):
    print(f"loading {fn}")
    vertices = []
    colors = []
    ...
    if sample_rate != 1:
        sample_indices = np.random.choice(
            vertices.shape[0], int(vertices.shape[0] * sample_rate), replace=False
        )
...
def load_mesh_obj(
    ...
    print(f"[mesh_io.load_mesh_obj] loading {fn}")
    mesh = o3d.io.read_triangle_mesh(fn)
    vertices = np.array(mesh.vertices)
    faces = np.array(mesh.triangles)
    if mesh.has_vertex_colors():
        print("[mesh_io.load_mesh_obj] Mesh has vertex colors")
        colors = np.asarray(mesh.vertex_colors)
    else:
        print("[mesh_io.load_mesh_obj] Mesh does not have vertex colors")
        colors = np.ones((len(mesh.vertices), 3))

    print(
        "[mesh_io.load_mesh_obj] loaded and simplified, face count:",
        len(faces),
        "vertex count:",
        len(vertices),
    )
    ...
    if reduction_rate != 1:
        ...
        sample_indices = np.random.choice(
            vertices.shape[0], int(vertices.shape[0] * reduction_rate), replace=False
        )
        vertices = vertices[sample_indices]
        vertices = (vertices, sample_indices)
        colors = colors[sample_indices]

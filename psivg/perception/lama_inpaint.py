...
from .ext.lama.saicinpainting.evaluation.data import pad_tensor_to_modulo
from .ext.lama.saicinpainting.evaluation.refinement import refine_predict
from .ext.lama.saicinpainting.evaluation.utils import move_to_device
from .ext.lama.saicinpainting.training.trainers import load_checkpoint
...
def mask_with_shade_new(img, mask, out_dir=None):
    ...
    num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(
        binary_inverted, connectivity=8
    )
...
def inpaint_img_with_builded_lama(
    model, img: np.ndarray, mask: np.ndarray, config_p=None, mod=8, device="cuda"
):
    assert len(mask.shape) == 2
    if np.max(mask) == 1:
        mask = mask * 255
...
if __name__ == "__main__":
    ...
    for mask_p in mask_ps:
        ...
        img_inpainted = inpaint_img_with_lama(
            ...
            args.lama_ckpt,
            device=device,
            dilation=args.dilate_kernel_size,
            find_shade=True,
        )

...
from vipe.utils.io import (ArtifactPath, read_depth_artifacts,
                           read_instance_artifacts, read_intrinsics_artifacts,
                           read_pose_artifacts, read_rgb_artifacts)
...
class ClientClosures:
    ...
    async def run(self):
        ...
        with self.client.gui.add_folder("Sample"):
            ...
            gui_id_changer = self.client.gui.add_button_group(
                label="ID +/-", options=["Prev", "Next"]
            )
            ...
            self.gui_t_sub = self.client.gui.add_slider(
                "Temporal subsample", min=1, max=16, step=1, initial_value=1
            )
            self.gui_s_sub = self.client.gui.add_slider(
                "Spatial subsample", min=1, max=8, step=1, initial_value=2
            )
        ...
        with self.client.gui.add_folder("Scene"):
            ...
            self.gui_fov = self.client.gui.add_slider(
                "FoV", min=30.0, max=120.0, step=1.0, initial_value=60.0
            )
            ...
            def _(_) -> None:
                ...
                snapshot_img = self.client.get_render(
                    height=720, width=1280, transport_format="png"
                )
                self.client.send_file_download(
                    file_name, iio.imwrite("<bytes>", snapshot_img, extension=".png")
                )
    ...
    def _set_frustum_color(self, colorful: bool):
        for frame_idx, frame_node in enumerate(self.scene_frame_handles):
            if not colorful:
                ...
                denom = len(self.scene_frame_handles)
                # rainbow_value = cm.jet(1.0 - frame_idx / denom)[:3]
                rainbow_value = cm.jet(frame_idx / denom)[:3]
    ...
    def _rebuild_scene(self):
        ...
        for frame_idx, (
            ...
            if depth is not None:
                pcd = (
                    rays * depth.numpy()[::spatial_subsample, ::spatial_subsample, None]
                )
                depth_mask = reliable_depth_mask_range(depth)[
                    ::spatial_subsample, ::spatial_subsample
                ].numpy()
                ist_mask = instance_mask[
                    ::spatial_subsample, ::spatial_subsample
                ].numpy()
                background_mask = ist_mask == 0
                foreground_mask = ist_mask > 0
                # combine depth_mask and background_mask
                # depth_mask = depth_mask & background_mask
            else:
    ...
    def _incr_timestep(self):
        if self.gui_timestep is not None:
            self.gui_timestep.value = (self.gui_timestep.value + 1) % len(
                self.scene_frame_handles
            )
    ...
    def _rebuild_playback_gui(self):
        ...
        with self.gui_playback_handle:
            self.gui_timestep = self.client.gui.add_slider(
                "Timeline",
                min=0,
                max=len(self.scene_frame_handles) - 1,
                step=1,
                initial_value=0,
            )
            gui_frame_control = self.client.gui.add_button_group(
                "Control", options=["Prev", "Next"]
            )
            self.gui_framerate = self.client.gui.add_slider(
                "FPS", min=0, max=30, step=1.0, initial_value=1
            )

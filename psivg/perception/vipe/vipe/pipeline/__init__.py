...
def make_pipeline(config: DictConfig, phrases: list[str] = []) -> Pipeline:
    ...
    if phrases and isinstance(phrases, list):
        print(f"Replacing phrases in config to provided phrases.")
        print(f"OLD phrases: {config.init.instance.phrases}")
        print(f"NEW phrases: {phrases}")
        config.init.instance.phrases = phrases
    ### Mark Added End

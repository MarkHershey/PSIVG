...
class GroundTruthMaskProcessor(StreamProcessor):
    """
    [IMPLEMENTED BY CURSOR]
    A processor that loads pre-computed ground truth masks and formats them
    exactly like TrackAnythingProcessor output for pipeline compatibility.
    """

    def __init__(
        self,
        gt_masks: list[torch.Tensor],  # List of (H, W) uint8 tensors, one per frame
        gt_phrases: dict[int, str],    # Mapping of instance ID to phrase
        add_sky: bool = False,
        mask_expand: int = 5,
    ) -> None:
        self.gt_masks = gt_masks
        self.gt_phrases = gt_phrases
        self.add_sky = add_sky
        self.mask_expand = mask_expand

        if self.add_sky:
            self.gt_phrases = self.gt_phrases.copy()
            self.gt_phrases[max(self.gt_phrases.keys()) + 1] = VideoFrame.SKY_PROMPT

    def update_attributes(self, previous_attributes: set[FrameAttribute]) -> set[FrameAttribute]:
        return previous_attributes | {FrameAttribute.INSTANCE, FrameAttribute.MASK}

    def __call__(self, frame_idx: int, frame: VideoFrame) -> VideoFrame:
        # Load ground truth mask for this frame
        if frame_idx < len(self.gt_masks):
            frame.instance = self.gt_masks[frame_idx].to(frame.device)
            frame.instance_phrases = self.gt_phrases.copy()
        else:
            # If no GT mask for this frame, create empty mask
            frame.instance = torch.zeros(frame.size(), dtype=torch.uint8, device=frame.device)
            frame.instance_phrases = {0: "background"}

        # Create mask exactly like TrackAnythingProcessor
